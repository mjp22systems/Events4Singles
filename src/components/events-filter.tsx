...
export default function EventsFilter({ cities, categories, selectedCity, selectedCategory, selectedPaid, view, month }: Props) {
  const router = useRouter();

  function go(city: string, cat: string, paid: string, v: string, m: string) {
  ...
  return (
    <div className="e4s-events-filter-bar">
      <div className="e4s-events-filter">
        <label>
          <span>City</span>
          <select value={c} onChange={(e) => go(e.target.value, cat, paid, view, month)}>
        ...
        <label>
          <span>Category</span>
          <select value={cat} onChange={(e) => go(c, e.target.value, paid, view, month)}>
        ...
        <label>
          <span>Price</span>
          <select value={paid} onChange={(e) => go(c, cat, e.target.value, view, month)}>
      ...
      <div className="e4s-events-view-toggle">
        <button
          ...
          onClick={() => go(c, cat, paid, "list", month)}
        ...
        <button
          ...
          onClick={() => go(c, cat, paid, "calendar", month)}
